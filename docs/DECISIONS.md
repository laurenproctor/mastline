...
- **The recipient reads the approval record, not the live asset.** Approval
  ...
  rewrites approved recipient content. Submissions from before the record
  were backfilled from their manifests with metadata as it stood at migration
  time and are marked `legacy_backfill` rather than passed off as the
  original approval. Settled 2026-08-28.
- **The dispatch screen's lifecycle stage is derived, not fixed.** It read
  ...
  Preparing → Review & approve → Create recipient link → Shared / awaiting
  outcome → Outcome recorded from the package status and the submission, with
  the existing vocabulary: approval is not sent, a link created is not shared,
  and an outcome is one that was recorded.
