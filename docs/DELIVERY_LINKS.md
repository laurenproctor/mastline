...
Previews are rendered from the **exact approved object** -- the version frozen
in the snapshot, scaled to 1400px on the long edge and marked -- never from
whichever preview or delivery derivative is preferred today. A frame whose
approved object is a RAW original therefore has no preview (`has_preview` is
false and the page draws its "No preview" state); nothing is substituted. If
the approved object cannot be read or marked, the route answers the same
neutral 404 and logs the frame id for the operator, without the storage
location.
...
Links created before migration `20260831090000` keep working where the
...
the submission screen says plainly. A submission whose manifest could not be
resolved receives no snapshot; its links open, show the terms, and list no
frames, and `submission_snapshot_gaps_admin()` names it for the operator.
