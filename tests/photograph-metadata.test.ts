...
import type { GeneratedMetadata } from "../src/lib/metadata-generation";
...
describeIf("editing and confirming", () => {
  ...
  it("marks the asset caption as reviewed when the confirmed record carried one", async () => {
    // The sentence the photographer confirmed in the panel is the sentence the
    // asset now carries, so the caption-review gate is satisfied by the same
    // act -- one read, one signature, not two.
    const { data: asset } = await serviceClient()
      .from("assets")
      .select("caption_origin, caption_awaits_review, caption_reviewed_at")
      .eq("id", assetId)
      .single();
    expect(asset?.caption_origin).toBe("human");
    expect(asset?.caption_awaits_review).toBe(false);
    expect(asset?.caption_reviewed_at).toBeTruthy();
  });

  it("does not launder an unread caption draft through a captionless confirmation", async () => {
    // The frame carries a model-drafted caption nobody has read, and the
    // metadata record being confirmed says nothing about a caption. Confirming
    // the record must leave the draft exactly as unread as it was: the
    // caption-review gate is cleared by reading the sentence, not by an
    // adjacent signature.
    const editor = await clientFor("editor");
    const { data: drafted, error } = await serviceClient()
      .from("assets")
      .insert({
        organization_id: ORG_A,
        shoot_id: shootId,
        status: "active",
        canonical_filename: `MH_DRAFTED_${Math.round(performance.now())}`,
        caption: "A machine's unread sentence.",
      ...
      .single();
    if (error) throw new Error(error.message);
    const draftedId = drafted!.id as string;
    await ensureMetadataRecord({ supabase: editor, organizationId: ORG_A, assetId: draftedId });
    ...
    const { data: after } = await serviceClient()
      .from("assets")
      .select("caption, caption_awaits_review")
      .eq("id", draftedId)
      .single();
    expect(after?.caption).toBe("A machine's unread sentence.");
    expect(after?.caption_awaits_review).toBe(true);
  });
