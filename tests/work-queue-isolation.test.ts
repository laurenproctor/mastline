...
async function linkedSubmission(label: string) {
  ...
  await service.from("delivery_access_events").insert({
    organization_id: ORG_A,
    delivery_id: link!.id as string,
    kind: "opened",
  });

  return { submissionId, token: link!.token as string };
}
...
describeIf("the dashboard and workspace boundaries", () => {
  it("contains nothing of another workspace, and never a delivery token", async () => {
    const { submissionId, token } = await linkedSubmission("WQISO");

    const owner = await clientFor("owner");
    const dashboard = await getWorkQueueDashboard(
      ORG_A,
      workspaceRoutes("marcus-hale-studio"),
      owner,
    );
    ...
    // what the page receives -- not in the activity, not in a link.
    ...
    // Previews, when present, are signed derivative URLs -- never originals.
    for (const shoot of dashboard.activeShoots) {
