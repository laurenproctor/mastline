 ...
 * builder prefers and therefore what gets frozen.
 */
async function readyShoot(
  label: string,
  options: { frames?: number; withDelivery?: boolean; organization?: "A" | "B" } = {},
) {
...
async function approved(label: string, options: { frames?: number; withDelivery?: boolean } = {}) {
...
describeIf("submissions approved before the snapshot existed", () => {
  ...
  it("are listed, not guessed at, when their manifest cannot be resolved", async () => {
    const service = serviceClient();
    const { shootId } = await readyShoot("GAP", { frames: 1 });
    const { data: pkg } = await service
      .from("packages")
      .insert({
        organization_id: ORG_A,
    ...
    // A manifest naming a version that does not exist: the shape of a legacy
    // row whose version was purged, or was never valid.
    ...
    const listed = (gaps.data ?? []).find(
      (row: { submission_id: string }) => row.submission_id === submission!.id,
    );
    expect(listed).toBeTruthy();
    expect(listed.manifest_asset_count).toBe(1);

    // Nothing was substituted: a link on it shows no frames.
    const link = await linkFor(submission!.id as string);
    const frames = await anonClient().rpc("delivery_assets", { delivery_token: link.token });
    expect(frames.data ?? []).toHaveLength(0);
    const opened = await anonClient().rpc("open_delivery", { delivery_token: link.token });
    expect(opened.data![0].asset_count).toBe(0);

    // The check functions are closed to everyone but the service role.
    const owner = await clientFor("owner");
    expect((await owner.rpc("submission_snapshot_gaps_admin")).error).toBeTruthy();
    expect((await anonClient().rpc("submission_snapshot_drift_admin")).error).toBeTruthy();
  });
});
