...
export async function GET(
  ...
  try {
    const { session, canonicalSlug } = await requireWorkspaceContext(workspace, "workspace.settings");
    ...
    const portal = await provider.createPortalSession(customerId, `${origin}/${canonicalSlug}/settings`);
