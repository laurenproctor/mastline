...
export async function approveAndSendAction(
  ...
  /*
   * The metadata records for exactly the photographs in this package.
   *
   * Loaded here and not trusted from the screen that rendered the button. The
   * page may have been open while a generation landed, and a caption a model
   * wrote in the meantime must not leave because the button was drawn before it
   * arrived. This is the last gate, so it re-reads everything it decides on.
   */
