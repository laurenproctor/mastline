...
export function ArchiveEmptyState({
  ...
  const sentence = state.query
    ? state.filter !== "all"
      ? `No assets match “${state.query}” with ${label} applied.`
      : `No assets match “${state.query}”.`
    : `No assets match the ${label} filter.`;

  return (
    <div className={styles.empty}>
      <h2 className={styles.emptyTitle}>Nothing matches</h2>
      <p>{sentence} The photographs are still here; loosen the search to see them.</p>
      <div className={styles.emptyActions}>
        {clearSearchHref && (
