/**
 * The contract with the model for structured photograph metadata, and the rules
 * that keep a generation a proposal.
 *
 * This is the structured sibling of src/lib/metadata-suggestions.ts. That
 * module drafts the caption trio the asset itself carries — headline, caption,
 * keywords — and stays exactly as it is: the caption writer at import and the
 * inspector's Suggest button both speak its contract. This one describes the
 * fuller record in asset_metadata: place, scene, visible objects and brands, a
 * category, a quality estimate, a sensitivity flag. Same model, same
 * conservatism, different destination.
 *
 ...
 *     used. `suggest -> explain -> confirm`, with the confirm step recorded
...
export { supportsEffort } from "./metadata-suggestions";
...
export function describeGenerationBasis(context: GenerationContext): string {
