...
export const BASELINE_RULES: readonly MetadataRule[] = [
   ...
   * exist.
   *
   * The caption rule above already refuses an unread caption draft on the
   * asset itself. This is the same principle applied to the structured record:
   * a machine may propose a scene, a city, a list of brands -- none of that may
   * reach a picture desk unread, because a desk will treat it as the
   ...
   * complete it looks. The same function also holds a frame while generation is
   * still running, while a sensitive flag is unconfirmed, and while a
   * record-level hold has not passed.
   *
...
export function reviewSelection(
  ...
  const reports = assets.map((asset) => reviewAsset(asset, rules, metadata?.get(asset.id) ?? null));
