...
describe("describeGenerationBasis", () => {
  it("always says that people are not identified", () => {
    expect(describeGenerationBasis({})).toContain("People are never identified by Mastline.");
  });

  it("names what it read, so the sentence in the panel is specific", () => {
    expect(describeGenerationBasis({ isVideo: true, shootTitle: "x", locationName: "y" })).toBe(
