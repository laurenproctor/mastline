...
export class ImportQueue {
  ...
  private async moveTo(
    ...
    const next = transition(record.status, status, record.clientFileId);
    return this.write({ ...record, ...changes, status: next, updatedAt: this.now });
  }
  ...
  private async stageOne(
    ...
    try {
      ...
      // The digest is taken from the copy that will actually be uploaded, and
      // is what finalization records against the original. Computing it now
      // means a session that resumes after a reload does not need the operator
      // to re-read a file it already has.
  ...
  async scheduleRetry(
    ...
    const record = await this.require(clientFileId);
    return this.view(
      await this.moveTo(record, "retrying", {
        errorCode: failure.code,
        errorMessage: sanitizeErrorMessage(failure.message),
  ...
  async recordUploadSession(clientFileId: string, uploadUrl: string): Promise<void> {
    const record = await this.require(clientFileId);
    if (record.uploadUrl === uploadUrl) return;
    await this.write({
      ...record,
      uploadUrl,
      uploadUrlCreatedAt: this.now,
      updatedAt: this.now,
    });
  }
