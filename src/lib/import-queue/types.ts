...
export interface ImportServerAdapter {
  ...
  markUploaded(input: {
    importFileId: Id;
    sha256: string;
    /** How many times this file has been attempted, so the row can say so. */
    attemptCount?: number;
  ...
  /**
   * Records that a file is waiting to be tried again.
   *
   * Distinct from reportFailure, which marks a file failed. Without this a
   * file that has been retrying for twenty minutes is indistinguishable in the
   * database from one nobody has started, which is exactly the question asked
   * of a stuck batch.
   */
  noteRetry(input: {
    importFileId: Id;
