...
export class FakeImportServer implements ImportServerAdapter {
  ...
  async markUploaded(input: { importFileId: Id; sha256: string }) {
    this.guard();
...
export class FakeTusClient {
  ...
  readonly script: Array<{ status?: number; body?: string; error?: Error }> = [];
    ...
    return (file, options) => {
      ...
      return {
        ...
        start() {
          queueMicrotask(() => {
            if (!url) {
              url = `https://storage.test/upload/${client.sessions.length + 1}`;
              client.sessions.push(url);
              options.onUploadUrlAvailable?.();
            }

            const held = client.serverOffsets.get(url) ?? 0;
            const total = (file as Blob).size;
            const next = client.script.shift();

            if (next) {
              const sent = held + Math.floor((total - held) * client.progressFraction * 0.5);
              options.onProgress?.(sent, total);
              client.serverOffsets.set(url, sent);
              options.onError?.(
                next.error ??
            ...
            client.serverOffsets.set(url, total);
            options.onSuccess?.();
          });
        },
      };
