  ...
  private async attempt(
    ...
    return new Promise<UploadResult>((resolve) => {
      ...
      const handle = factory(blob, {
        ...
        // plainly had one.
        onSuccess: () => finish({ ok: true, uploadUrl: handle.url ?? uploadUrl, bytesUploaded }),
        onError: (error) =>
          finish({
            ok: false,
...
/** Pull the status and body out of whatever tus threw. */
function describe(error: unknown): { error: unknown; status?: number; body?: string } {
  ...
  return { error, status: response.getStatus(), body };
}
...
export async function loadTusFactory(): Promise<TusUploadFactory> {
  ...
  return (file, options) =>
    new tus.Upload(file, options as never) as unknown as TusUploadHandle;
}
