...
export function ImportQueuePanel({
  ...
  const [items, setItems] = useState<readonly QueueItemView[]>([]);
  const [busy, setBusy] = useState<string | null>(null);

  /**
   * Every snapshot is merged, not just the one that happens to be rendered.
   *
   * A completed import is cleaned up: the local copy is deleted and the record
   * goes with it, which is right -- the asset is the record now. But the row
   * would go too, so a batch of forty would shrink to nothing and report "0 of
   * 0 imported", which is the opposite of what somebody watching it wants.
   *
   * Merging in the subscription rather than during render matters, because
   * React is free to coalesce renders: the snapshot where a file is complete
   * may never be rendered at all, and remembering it must not depend on that.
   */
  const seen = useRef(new Map<string, QueueItemView>());
  const merge = useCallback(
    (next: QueueSnapshot) => {
      setSnapshot(next);
      const mine = batchId ? next.items.filter((item) => item.batchId === batchId) : next.items;
      const live = new Set(mine.map((item) => item.clientFileId));

      for (const item of mine) seen.current.set(item.clientFileId, item);
      // Anything that left the queue for a reason other than finishing is
      // genuinely gone: cancelled, or cleaned up unfinished.
      for (const [id, item] of [...seen.current]) {
        if (!live.has(id) && item.status !== "complete") seen.current.delete(id);
      }

      setItems([...seen.current.values()].sort((a, b) => a.createdAt.localeCompare(b.createdAt)));
    },
    [batchId],
  );

  useEffect(() => {
    const unsubscribe = session.subscribe(merge);
    void session.ready();
    void session.runner.snapshot().then(merge);
    return unsubscribe;
  }, [merge, session]);
          ...
          return (
            <li className={`import-row ${item.status}`} key={item.clientFileId}>
              ...
              <span className="import-state">
                ...
                {item.status === "uploading" || item.status === "retrying" ? ` · ${percent}%` : ""}
              ...
              <span className="import-actions">
                {item.needsFile && (
                  <button className="button small" onClick={() => onFileNeeded(item)} type="button">
                    Choose file
                  </button>
                )}
                ...
                {!item.needsFile && IN_FLIGHT.includes(item.status) && (
                  <button
                    ...
                    disabled={working}
                    onClick={() =>
                      void act(item.clientFileId, () => session.runner.pauseItem(item.clientFileId))
                    }
                    type="button"
                  >
                ...
                {item.status === "paused" && (
                  <button
                    ...
                    disabled={working}
                    onClick={() =>
                      void act(item.clientFileId, () =>
                        session.runner.resumeItem(item.clientFileId),
                      )
                    }
                    type="button"
                  >
                ...
                {item.status === "failed" && !item.needsFile && (
                  <button
                    ...
                    disabled={working}
                    onClick={() =>
                      void act(item.clientFileId, () => session.runner.retryItem(item.clientFileId))
                    }
                    type="button"
                  >
                ...
                {item.status !== "complete" && item.status !== "canceled" && (
                  <button
                    ...
                    onClick={() =>
                      void act(item.clientFileId, () =>
                        session.runner.cancelItem(item.clientFileId),
                      )
                    }
                    type="button"
                  >
